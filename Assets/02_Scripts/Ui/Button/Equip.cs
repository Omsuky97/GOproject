            ...
            if (relic_id != 0)
            {
                for (int Relic_Num = 0; Relic_Num < Relic.Count; Relic_Num++)
                {
                    if (relic_id_num[Relic_Num] == relic_id)
                    {
                        ...
                        if (Relic[Relic_Num].Bullet_Spirt_Type) Bullet_Manager.Instance.Bullet_Spirt_Type = false;         //총알 분열체 수 증가
                        if (Relic[Relic_Num].Bullet_Pec) Bullet_Manager.Instance.Bullet_Pec_Type = false;
                        if (Relic[Relic_Num].Bullet_Speaker_Type) Bullet_Manager.Instance.Bullet_Speaker_Type = false;    //총알 연속발사 확률
                        if (Relic[Relic_Num].Bullet_Return) Bullet_Manager.Instance.Bullet_Boomerang_Type = false;            //총알 타겟 명중 후 되돌아오는 여부
                        if (Relic[Relic_Num].Bullet_Bounce) Bullet_Manager.Instance.Bullet_bounce_Type = false;           //가장멀리있는 적 사격 여부
                        if (Relic[Relic_Num].Bullet_Sniping) Bullet_Manager.Instance.Bullet_Target_type = false;           //가장멀리있는 적 사격 여부
                        if (Relic[Relic_Num].Bullet_Gra_Speed) Bullet_Manager.Instance.Bullet_Propulsion_Type = false;         //총알 점진적 이동 여부
                        if (Relic[Relic_Num].Bullet_Bomb) Bullet_Manager.Instance.Bullet_Boom_Type = false;              //총알 타겟 명중 시 범위 피해 여부
                        if (Relic[Relic_Num].Bullet_Idt) Bullet_Manager.Instance.Bullet_Guided_Type = false;
                        if (Relic[Relic_Num].Bullet_Noc) Bullet_Manager.Instance.Bullet_NucBack_Type = false;               //총알 넉백 여부 --추가
                        if (Relic[Relic_Num].Bullet_Effec_Type) Bullet_Manager.Instance.Bullet_Bezier_Type = false;         //총알 효과 트리거 횟수
                        if (Relic[Relic_Num].Bullet_ShotGun_Type) Bullet_Manager.Instance.Bullet_ShotGun_Type = false;         //총알 넉백 여부 --추가                    //총알 넉백 여부 --추가
                        exitLoop = true;
            ...
            if (relic_id_num[Slot_Count] == 0) return;
                ...
                for (int Relic_NUm = 0; Relic_NUm < Relic.Count; Relic_NUm++)
                {
                    if (Relic[Relic_NUm].Relics_id == relic_id_num[Slot_Count])
                    {
                        ...
                        if (Relic[Relic_NUm].Bullet_Noc) Bullet_Manager.Instance.Bullet_NucBack_Type = false;               //총알 넉백 여부 --추가
                        if (Relic[Relic_NUm].Bullet_Effec_Type) Bullet_Manager.Instance.Bullet_Bezier_Type = false;         //총알 효과 트리거 횟수
                        if (Relic[Relic_NUm].Bullet_ShotGun_Type) Bullet_Manager.Instance.Bullet_ShotGun_Type = false;         //총알 넉백 여부 --추가          
                    }
                    if (Relic[Relic_NUm].Relics_id == relic_id_num[Slot_Count])
                    {
                        GameManager.Instance.bullet_damage += (GameManager.Instance.bullet_damage * Relic[Relic_NUm].Bullet_Pow_UP[Relic[Relic_NUm].Relics_Lv - 1]);   //공격력 증가
